...
function detectMimeMagic(bytes: Uint8Array): string {
  ...
  if (bytes.length >= 4 &&
      ...
      bytes[2] === 0x53 && bytes[3] === 0x20) return "image/vnd.ms-dds";
...
async function createScene(
  ...
  const convertMat = (mat: THREE.Material, mode: ShaderMode): THREE.Material => {
    ...
    if (mode === "pbr") {
      return new THREE.MeshStandardMaterial({
        ...base,
        roughness: 0.72,
        metalness: p?.specular ? Math.min(0.06, p.specular.r * 0.06) : 0.02,
      });
    }
  ...
  const applyShader = (mode: ShaderMode) => {
    ...
    modelRoot.traverse((child: THREE.Object3D) => {
      ...
      if (!originalMats.has(mesh)) {
        originalMats.set(mesh, Array.isArray(mesh.material) ? [...mesh.material] : [mesh.material]);
      }
      const originals = originalMats.get(mesh)!;
      mesh.material = originals.length === 1 ? convertMat(originals[0], mode) : originals.map((m) => convertMat(m, mode));
      if (mode !== "pbr" && !((mesh as any).isSkinnedMesh)) {
  ...
  const keyLight = new THREE.DirectionalLight(0xfff4e0, 4.5); keyLight.position.set(3, 6, 4); keyLight.castShadow = true; keyLight.shadow.mapSize.set(1024, 1024); scene.add(keyLight);
  const fillLight = new THREE.DirectionalLight(0xb8d0ff, 2.0); fillLight.position.set(-4, 3, 1); scene.add(fillLight);
  const rimLight = new THREE.DirectionalLight(0xffffff, 2.5); rimLight.position.set(0, 4, -5); scene.add(rimLight);
  const bounceLight = new THREE.DirectionalLight(0xffeedd, 0.8); bounceLight.position.set(0, -3, 2); scene.add(bounceLight);
  const grid = new THREE.GridHelper(10, 20, 0x333333, 0x222222); scene.add(grid);
  ...
  const textureBlobCache = new Map<string, string>();
  const inTauri = typeof window !== "undefined" && !!(window as any).__TAURI_INTERNALS__;
    ...
    if (inTauri) {
      try {
        ...
        for (const dir of dirsToScan) {
          ...
          const entries = await readDir(dir).catch(() => [] as any[]);
          for (const entry of entries as any[]) {
            if (entry.isDirectory) await cacheDir(`${dir}${sep}${entry.name}`);
          }
        }
      } catch {}
    }
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex items-center gap-2">
        <span className="text-[10px] text-zinc-500 shrink-0">{t("preview_3d_shader")}</span>
        <div className="flex rounded-md overflow-hidden border border-zinc-700 text-[10px]">
          {([ { id: "pbr", label: "PBR" }, { id: "liltoon", label: "lilToon" }, { id: "poiyomi", label: "Poiyomi" } ] as { id: ShaderMode; label: string }[]).map(({ id, label }) => (
            <button
              key={id}
        ...
        {loading && (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 bg-zinc-900/80 pointer-events-none">
            ...
            <p className="text-xs text-zinc-500">
              {t("preview_3d_loading", { model: modelName(selectedModel) })} {loadProgress > 0 ? `${loadProgress}%` : ""}
            </p>
          </div>
        )}
        ...
        {error && !loading && (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 p-4 pointer-events-none">
            ...
            {error.includes("npm install") && (
              <code className="text-[10px] bg-zinc-800 text-amber-300 px-3 py-1.5 rounded-lg font-mono">{t("preview_3d_install_hint")}</code>
            )}
          </div>
        )}
