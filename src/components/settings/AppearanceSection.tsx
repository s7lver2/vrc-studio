...
const ACCENT_BTN = (active: boolean) =>
    active
...
function ThemePreviewCard({ theme, isActive, onSelect }: { theme: AppTheme; isActive: boolean; onSelect: () => void }) {
    ...
    return (
        <button
            ...
            style={{
                borderRadius: "14px",
                border: isActive ? `2px solid ${accentColor}` : `2px solid ${theme.borderColor}`,
                boxShadow: isActive ? `0 0 0 3px ${accentColor}18` : "none",
                transform: isActive ? "scale(1.03)" : "scale(1)",
            }}
        >
            <div className="w-full" style={{ height: 52, background: theme.appBg, display: "flex", alignItems: "flex-end", padding: "0 10px 8px", gap: 4 }}>
                <div style={{ width: 6, height: 28, borderRadius: 3, background: theme.sidebarBg, border: `1px solid ${theme.borderColor}` }} />
                {[0, 1, 2].map((i) => (
                    <div key={i} style={{ flex: 1, height: i === 0 ? 28 : 20, borderRadius: Number(theme.radiusSm.replace("px", "")) > 4 ? 6 : 2, background: theme.cardBg, border: `1px solid ${theme.borderColor}`, alignSelf: "flex-end" }} />
                ))}
                <div style={{ width: 7, height: 7, borderRadius: "50%", background: accentColor, alignSelf: "flex-start", marginTop: 8, flexShrink: 0, boxShadow: `0 0 6px ${accentColor}80` }} />
            </div>
            <div style={{ background: theme.cardBg, borderTop: `1px solid ${theme.borderColor}`, padding: "7px 10px", display: "flex", alignItems: "center", gap: 5 }}>
                <span style={{ fontSize: 12 }}>{theme.emoji}</span>
                <span style={{ fontSize: 10, fontWeight: 700, color: isActive ? accentColor : theme.textPrimary, letterSpacing: "-0.01em" }}>{theme.name}</span>
                {isActive && <div style={{ marginLeft: "auto", width: 6, height: 6, borderRadius: "50%", background: accentColor, boxShadow: `0 0 4px ${accentColor}` }} />}
            </div>
        </button>
    );
}
    ...
    return (
        <div className="flex flex-col gap-4">
            ...
            <div
                ...
                style={{ minHeight: 120, background: "#08080c", borderColor: isActive ? "rgba(255,255,255,0.12)" : "rgb(39,39,42)" }}
                onClick={handlePickFile}
            >
                {previewSrc ? (
                    <>
                        {wallpaper.mediaType === "video" ? (
                            <video src={previewSrc} className="w-full object-cover" style={{ height: 130, filter: wallpaper.blur > 0 ? `blur(${wallpaper.blur * 0.25}px)` : undefined, opacity: isActive ? 1 : 0.35 }} autoPlay loop muted playsInline />
                        ) : (
                            <img src={previewSrc} alt="" className="w-full object-cover" style={{ height: 130, filter: wallpaper.blur > 0 ? `blur(${wallpaper.blur * 0.25}px)` : undefined, opacity: isActive ? 1 : 0.35 }} />
                        )}
                        <div className="absolute inset-0 pointer-events-none" style={{ background: `rgba(0,0,0,${isActive ? wallpaper.overlayOpacity : 0.6})` }} />
                    ...
                    <div className="flex flex-col items-center justify-center h-28 gap-3 text-zinc-700 group-hover:text-zinc-500 transition-colors">
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-[10px] text-zinc-600 mt-0.5">Images, animated GIFs and MP4 video</p>
                </div>
                <div className="flex items-center gap-2">
                    {wallpaper.path && (
                        <button onClick={() => setWallpaper({ path: null, enabled: false })} className="text-[10px] text-zinc-600 hover:text-red-400 transition-colors px-2 py-1 rounded-lg hover:bg-red-500/10">
                            Remove
                        </button>
                    )}
                    <button
                ...
                <div className="flex flex-col gap-4 rounded-xl border border-zinc-800 bg-zinc-900/40 p-4">
                    <div className="grid grid-cols-2 gap-4">
                        <div className="flex flex-col gap-2">
                            ...
                            <input type="range" min={5} max={90} step={5} value={Math.round(wallpaper.overlayOpacity * 100)} onChange={(e) => setWallpaper({ overlayOpacity: Number(e.target.value) / 100 })} className="w-full cursor-pointer h-1.5 rounded-full appearance-none" style={{ accentColor: "var(--accent-color)" }} />
                        </div>
                        <div className="flex flex-col gap-2">
                            <div className="flex justify-between items-center">
                            ...
                            <input type="range" min={0} max={20} step={1} value={wallpaper.blur} onChange={(e) => setWallpaper({ blur: Number(e.target.value) })} className="w-full cursor-pointer h-1.5 rounded-full appearance-none" style={{ accentColor: "var(--accent-color)" }} />
                        </div>
                    </div>
                    <div className="flex flex-col gap-1.5">
                        ...
                        <div className="flex gap-1.5">
                            {(["cover", "contain", "tile"] as const).map((f) => (
                                <button key={f} onClick={() => setWallpaper({ fit: f })} className="flex-1 py-1.5 rounded-lg border text-[10px] font-medium capitalize transition-all" style={ACCENT_BTN(wallpaper.fit === f)}>{f}</button>
                            ))}
                        </div>
                    </div>
                    <div className="flex flex-col gap-2">
                        <div className="flex items-center justify-between">
                            <label className="text-[10px] text-zinc-500 uppercase tracking-wider font-semibold">Visible in sections</label>
                            <button onClick={() => { const allSelected = ALL_SECTIONS.every(s => wallpaper.visibleSections.includes(s)); setWallpaper({ visibleSections: allSelected ? [] : [...ALL_SECTIONS] }); }} className="text-[10px] text-zinc-600 hover:text-zinc-400 transition-colors">
                                {ALL_SECTIONS.every(s => wallpaper.visibleSections.includes(s)) ? "Deselect all" : "Select all"}
                            </button>
                        </div>
                        <div className="grid grid-cols-2 gap-1">
                            {ALL_SECTIONS.map((section) => {
                                const active = wallpaper.visibleSections.includes(section);
                                return (
                                    <button key={section} onClick={() => { const next = active ? wallpaper.visibleSections.filter(s => s !== section) : [...wallpaper.visibleSections, section]; setWallpaper({ visibleSections: next }); }} className="flex items-center gap-2 px-2.5 py-1.5 rounded-lg border text-[10px] font-medium transition-all text-left" style={active ? { borderColor: "var(--accent-color)", background: "hsl(var(--accent-h) var(--accent-s) var(--accent-l) / 0.10)", color: "var(--accent-color)" } : { borderColor: "transparent", background: "rgb(39,39,42)", color: "rgb(113,113,122)" }}>
                                        <div className="w-1.5 h-1.5 rounded-full shrink-0 transition-colors" style={{ background: active ? "var(--accent-color)" : "rgb(63,63,70)" }} />
...
function CarouselImageManager() {
    ...
    const handleAddCustom = async () => {
        const file = await tauriOpenDialog({ multiple: false, filters: [{ name: "Images", extensions: ["jpg", "jpeg", "png", "webp"] }] });
        if (!file || Array.isArray(file)) return;
        addCarouselImage({ id: crypto.randomUUID(), path: file as string, builtInId: null });
    };
        ...
        <div className="flex flex-col gap-3 rounded-xl border border-zinc-800 bg-zinc-900 p-4">
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1.5">
                    <button onClick={() => setCarouselImages(BUILT_IN_SPLASH_IMAGES.map(img => ({ id: img.id, path: null, builtInId: img.id })))} className="text-[10px] text-zinc-500 hover:text-zinc-300 transition-colors px-2 py-1 rounded border border-zinc-700 hover:border-zinc-500">
                        Use all built-in
                    </button>
                    <button onClick={handleAddCustom} className="flex items-center gap-1 text-[10px] font-medium px-2 py-1 rounded border transition-colors" style={{ borderColor: "var(--accent-color)", color: "var(--accent-color)", background: "hsl(var(--accent-h) var(--accent-s) var(--accent-l) / 0.1)" }}>
            ...
            {carouselImages.length > 0 && (
                <div className="flex flex-col gap-1.5 max-h-40 overflow-y-auto">
                    {carouselImages.map((entry) => {
                        ...
                        return (
                            <div key={entry.id} className="flex items-center gap-2.5 px-3 py-2 rounded-lg border border-zinc-800 bg-zinc-800/50">
                                <div className="w-10 h-7 rounded overflow-hidden shrink-0 bg-zinc-700">
                                    ...
                                    {entry.path && !meta && <div className="w-full h-full flex items-center justify-center text-[8px] text-zinc-500">IMG</div>}
                                </div>
                                <div className="flex-1 min-w-0">
                                    <p className="text-[10px] font-medium text-zinc-200 truncate">{meta ? meta.title : "Custom image"}</p>
                                    <p className="text-[9px] text-zinc-600 truncate">{meta ? `by ${meta.artist}` : entry.path?.split(/[\\/]/).pop()}</p>
                                </div>
                                <button onClick={() => removeCarouselImage(entry.id)} className="text-zinc-600 hover:text-red-400 transition-colors shrink-0">
            ...
            <div>
                ...
                <div className="grid grid-cols-4 gap-1.5">
                    {BUILT_IN_SPLASH_IMAGES.map((img) => {
                        ...
                        return (
                            <button key={img.id} onClick={() => { if (!already) addCarouselImage({ id: crypto.randomUUID(), path: null, builtInId: img.id }); }} disabled={already} className="relative aspect-video rounded overflow-hidden border-2 transition-all disabled:opacity-40" style={already ? { borderColor: "var(--accent-color)" } : { borderColor: "transparent" }} title={`${img.title} — ${img.artist}`}>
                                <img src={img.url} alt={img.title} className="w-full h-full object-cover" />
                                {already && <div className="absolute inset-0 flex items-center justify-center" style={{ background: "rgba(0,0,0,0.5)" }}><span className="text-[8px] font-bold text-white">✓</span></div>}
                            </button>
                        );
                    })}
                </div>
...
export function AppearanceSection() {
    ...
    return (
        <div className="flex flex-col gap-8">
                    ...
                    <>
                        ...
                        <div className="flex gap-3">
                            <button
                                ...
                                style={loadingScreen === "classic" ? { borderColor: "var(--accent-color)", background: "hsl(var(--accent-h) var(--accent-s) var(--accent-l) / 0.08)" } : { borderColor: "rgb(39,39,42)", background: "rgb(24,24,27)" }}
                            >
                            ...
                            <button
                                ...
                                className="flex-1 flex flex-col gap-2 p-3 rounded-xl border-2 transition-all text-left"
                                style={loadingScreen === "carousel" ? { borderColor: "var(--accent-color)", background: "hsl(var(--accent-h) var(--accent-s) var(--accent-l) / 0.08)" } : { borderColor: "rgb(39,39,42)", background: "rgb(24,24,27)" }}
                            >
                                <div className="w-full aspect-video rounded-lg overflow-hidden relative" style={{ background: "#1a1a2e" }}>
                                    ...
                                    <div className="absolute right-2 top-2 bottom-2 flex flex-col items-center justify-center gap-1" style={{ width: 16, background: "rgba(0,0,0,0.4)", borderRadius: 4 }}>
                                        ...
                                        <div className="w-0.5 h-8 rounded-full" style={{ background: "rgba(255,255,255,0.2)" }}>
                                            <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: "60%", background: "rgba(255,255,255,0.7)", borderRadius: 99 }} />
                                        </div>
                                    </div>
                                ...
                                <div>
                                    <p className="text-xs font-semibold text-zinc-200">{t("appearance_loading_carousel")}</p>
                                    <p className="text-[10px] text-zinc-500 mt-0.5">{t("appearance_loading_carousel_desc")}</p>
                                </div>
                            </button>
                        </div>
                                        ...
                                        <div>
                                            ...
                                            <p className="text-[10px] text-zinc-500 mt-0.5">
                                                {vrchatGallery.consented
                                                    ? vrchatGallery.folderPath || "Configured"
                                                    : "Use your VRChat photos in the carousel"}
                                            </p>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-2 shrink-0">
                                        {vrchatGallery.consented ? (
                                            <>
                                                ...
                                                <button onClick={() => setVRChatGallery({ consented: false, enabled: false, folderPath: "" })} className="p-1 rounded hover:bg-zinc-800 text-zinc-600 hover:text-zinc-300 transition-colors" title="Revoke access">
                                                    <XIcon className="h-3.5 w-3.5" />
                                                </button>
                                            </>
                                        ) : (
                                            <button onClick={() => setShowVRChatConsent(true)} className="px-3 py-1.5 text-xs rounded-lg bg-violet-900/40 hover:bg-violet-900/60 border border-violet-700/50 text-violet-300 transition-colors">
                                                Configure
                                            </button>
                                        )}
                                    </div>
            ...
            {/* ── LAYOUT ── */}
            <div className="flex flex-col gap-3">
                <SectionLabel icon={Monitor}>{t("appearance_layout_text_label")}</SectionLabel>
                <div className="rounded-xl border border-zinc-800 bg-zinc-900 overflow-hidden divide-y divide-zinc-800">
                    ...
                    <div className="px-5 py-4 flex items-center gap-4">
                        ...
                        <div className="flex gap-1">
                            {([80, 90, 100, 110, 120] as const).map((pct) => (
                                <button key={pct} onClick={() => setUiScale(pct / 100 as any)} className="px-2 py-1 rounded-lg border text-[10px] font-semibold transition-all" style={ACCENT_BTN(uiScale === pct / 100)}>{pct}%</button>
                            ))}
                        </div>
                    </div>
                    {/* Font Size */}
                    <div className="px-5 py-4 flex items-center gap-4">
                        ...
                        <div className="flex gap-1">
                            {([{ value: "small", label: "S" }, { value: "normal", label: "M" }, { value: "large", label: "L" }] as const).map((opt) => (
                                <button key={opt.value} onClick={() => setFontSize(opt.value)} className="w-9 py-1.5 rounded-lg border text-[10px] font-semibold transition-all" style={ACCENT_BTN(fontSize === opt.value)}>{opt.label}</button>
                            ))}
                        </div>
                    </div>
                    {/* Animation Speed */}
                    <div className="px-5 py-4 flex items-center gap-4">
                        <div className="flex-1">
                            <p className="text-sm font-medium text-zinc-200">{t("appearance_anim_speed_label")}</p>
                            <p className="text-[10px] text-zinc-600 mt-0.5">Transition speed for UI elements</p>
                        </div>
                        <div className="flex gap-1">
                            {(["off", "slow", "normal", "fast"] as const).map((opt) => {
                                const emoji = { off: "🚫", slow: "🐢", normal: "⚡", fast: "🚀" }[opt];
                                return (
                                    <button key={opt} onClick={() => setAnimSpeed(opt)} className="flex flex-col items-center gap-0.5 px-2 py-1.5 rounded-lg border text-[9px] font-semibold capitalize transition-all" style={ACCENT_BTN(animSpeed === opt)}>
                    ...
                    <div className="px-5 py-4 flex items-center gap-4">
                        ...
                        <div className="flex gap-1.5">
                            {(["grid", "list"] as const).map((v) => (
                                <button key={v} onClick={() => setDefaultView(v)} className="px-3 py-1.5 rounded-lg border text-xs font-medium capitalize transition-all" style={ACCENT_BTN(defaultView === v)}>{v}</button>
                            ))}
                        </div>
                    </div>
                    {/* Show tags */}
            ...
            {/* ── CARD SIZES ── */}
            <div className="flex flex-col gap-3">
                <SectionLabel icon={LayoutGrid}>{t("appearance_card_sizes_label")}</SectionLabel>
                        ...
                        <div key={key} className="flex items-center gap-3 flex-1">
                            ...
                            <div className="flex gap-1 flex-1">
                                {(["compact", "normal", "large"] as const).map((size) => (
                                    <button key={size} onClick={() => setter(size)} className="flex-1 py-1.5 rounded-lg border text-[10px] font-semibold transition-all" style={ACCENT_BTN(value === size)}>
            ...
            {showVRChatConsent && (
                <VRChatGalleryConsentModal onClose={() => setShowVRChatConsent(false)} onConsented={() => setShowVRChatConsent(false)} />
            )}
        </div>
    );
}
