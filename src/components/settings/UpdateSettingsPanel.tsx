...
export function UpdateSettingsPanel() {
  ...
  const onBeta = isBetaChannel(settings.channel);
  ...
  const handleRedeemBeta = useCallback(async () => {
    ...
    try {
      ...
      setBetaSubs((prev) => [sub, ...prev.filter((s) => s.slug !== sub.slug)]);
      setBetaCode("");
    } catch {
  ...
  const handleLeaveBeta = useCallback(async (slug: string) => {
    try { await invoke("remove_beta_subscription", { slug }); } catch { /* ignore */ }
    setBetaSubs((prev) => prev.filter((s) => s.slug !== slug));
    if (settings.channel === slug) updateSettings({ channel: "stable", betaBuild: undefined });
  ...
  const handleInstallVersion = useCallback(async (v: AvailableVersion) => {
    ...
    try {
      await invoke("download_and_install_update", {
        url: v.download_url, signature: "", channel: settings.channel,
      });
    } catch (e) {
      setVersionsError(String(e));
    } finally {
  ...
  // ── Channel meta (only stable/testing) ──────────────────────────────────
  ...
  // Safe label for any channel (including beta slugs)
  const channelLabel = onBeta
  ...
  return (
    <div className="space-y-6 text-sm text-white/80">
      ...
      <section>
        ...
        <div className="flex gap-2">
          {(["stable", "testing"] as const).map((c) => {
            ...
            const active = !onBeta && settings.channel === c;
            return (
              <button
                key={c}
          ...
          <div className="mt-2 rounded-lg border border-purple-500/40 bg-purple-500/10 px-3 py-2
            ...
            <div>
              <p className="text-xs text-purple-300 font-medium">
                ...
                <span className="text-white">{channelLabel}</span>
              </p>
      ...
      <section>
        ...
        {!checking && !updateInfo && !error && (
          <p className="mt-2 text-white/40 text-xs">
            {t("updates_up_to_date", { channel: channelLabel })}
          </p>
        )}
      </section>
      ...
      <section>
        ...
        {versionsError && <p className="text-red-400 text-xs mb-2">{versionsError}</p>}

        {versions && (
          <div className="space-y-1.5 max-h-60 overflow-y-auto pr-1">
            {versions.length === 0 && (
              <p className="text-white/40 text-xs">
                {t("updates_versions_empty", { channel: channelLabel })}
              </p>
            )}
            {versions.map((v) => (
              <div
              ...
              >
                <div className="min-w-0">
                  <span className="font-medium text-white text-xs">
                    {onBeta ? `Build #${v.version}` : `v${v.version}`}
                  </span>
                  {v.is_current && (
      ...
      <section>
        ...
        {betaError && <p className="text-red-400 text-xs mb-3">{betaError}</p>}
                  ...
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      <p className={`font-medium text-xs ${isActive ? "text-purple-300" : "text-white"}`}>
                        {sub.name}
                        {isActive && (
                          <span className="ml-2 text-[10px] text-purple-400 font-normal">● activa</span>
                        )}
                      </p>
                      {sub.description && (
                      ...
                      <p className="text-white/20 text-[10px] mt-1 font-mono">{sub.slug}</p>
                    </div>
